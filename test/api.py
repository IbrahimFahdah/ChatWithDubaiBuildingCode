"""
FastAPI RAG server — Dubai Building Code (local / test).
...
async def lifespan(app: FastAPI):
    if not INDEX_PATH.exists() or not META_PATH.exists():
        raise RuntimeError("Index files not found. Re-run the Colab notebook.")
...
app = FastAPI(title="Dubai Building Code RAG — Test", lifespan=lifespan)
    ...
    try:
        r = requests.post(
            ...
            json={
                ...
                "options":  {"temperature": 0.1, "num_predict": 512, "repeat_penalty": 1.3},
            },
            timeout=120,
        )
        if not r.ok:
            raise HTTPException(502, f"Ollama chat failed ({r.status_code}): {r.text.strip()}")
...
def ask(req: AskRequest):
    ...
    answer = call_ollama(build_prompt(req.question, chunks))
    return AskResponse(
        answer=answer,
        sources=[
